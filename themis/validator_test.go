...
func TestMatchClientHostnames(t *testing.T) {
	...
	assert.Equal(t, false, matchClientHostnames("192.168.0.0", "*.168.0.0"), "Hostname validation Failed is expected")
	...
	assert.Equal(t, true, matchClientHostnames("ww.test3.com", "ww*.test3.com"), "Hostname validation SUCCESS is expected")
	...
	assert.Equal(t, false, matchClientHostnames("*.168.0.0", "192.168.0.0"), "Hostname validation Failed is expected")
	assert.Equal(t, false, matchClientHostnames("192.168.0.0", "*.168.0.0"), "Hostname validation Failed is expected")
